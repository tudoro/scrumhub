...
Meteor.methods({
    ...
    setActiveUserInHub: function(hub, activeUser) {
        ...
        if (Meteor.user() && (hub.owner === Meteor.userId() || activeUser.meetupUser === Meteor.userId())) {
            ...
            if (Meteor.isServer) {
                var workedOnIssues = Meteor.call("apimethods_getWorkedOnIssues", hub);
                var currentlyWorkingOnIssues = Meteor.call("apimethods_getWorkingOnIssues", hub);
                Hubs.update({_id: hub._id}, {
                    $set: {
                        activeWorkedOnIssues: workedOnIssues.data.issues,
                        currentlyWorkingOnIssues: currentlyWorkingOnIssues.data.issues
                    }
                });
            }
